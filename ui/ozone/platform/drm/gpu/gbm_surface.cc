...
GbmSurface::GbmSurface(DrmWindow* window_delegate,
    ...
    : GbmSurfaceless(window_delegate, NULL),
      ...
      weak_factory_(this) {
  LOG(INFO) << "*** CAS GbmSurface gbm " << gbm.get();
}
...
bool GbmSurface::Initialize() {
  ...
  LOG(INFO) << "Initializing gbm native surface " << native_surface_;
  if (!native_surface_)
    return false;
...
EGLNativeWindowType GbmSurface::GetNativeWindow() {
  DCHECK(native_surface_);
  return reinterpret_cast<EGLNativeWindowType>(native_surface_);
}

bool GbmSurface::ResizeNativeWindow(const gfx::Size& viewport_size) {
  LOG(INFO) << "GbmSurface::ResizeNativeWindow " << size_.width() << " " << size_.height();
  if (size_ == viewport_size) {
    LOG(INFO) << "ResizeNativeWindow returning true";
    return true;
  }

  LOG(INFO) << "ResizeNativeWindow returning false";
  return false;
}
